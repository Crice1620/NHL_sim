...
DEF_PROXY_SCALE <- 3         # max approx +/- shots/game swing from the blocks/hits defensive proxy — tune as needed
HOME_SHOT_BOOST <- 1.0       # extra shots/game for the home team (mirrors a modest home-ice edge)
HOME_GOAL_BOOST <- 0.01      # small additive bump to the home team's per-shot goal probability
...
team_offense <- team_offense %>%
  mutate(
    ...
    shots_against_pg = pmax(15, LEAGUE_AVG_SHOTS_PG - def_z * DEF_PROXY_SCALE)
  )
