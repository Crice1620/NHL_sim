...
all_rosters <- bind_rows(lapply(team_abbrevs, function(a) {
  # Prefer "current" — this is the team's live roster right now, so any
  # trade or free-agent signing already completed this offseason shows up
  # immediately, rather than waiting for the NHL to formally publish next
  # season's roster object.
  ...
  if (is.null(r)) {
    # Last resort: most recently completed season's roster as a proxy.
