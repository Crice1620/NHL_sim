...
project_skater_stats <- function(hist_df) {
  ...
  rates %>%
    ...
    summarise(
      ...
      position    = dplyr::last(position),
      n_seasons   = dplyr::n(),
      proj_gp     = { w <- recency_weights(season); min(FULL_SEASON_GP, round(sum(w * gp))) },
      rate_goals   = { w <- recency_weights(season); sum(w * r_goals) },
      rate_assists = { w <- recency_weights(season); sum(w * r_assists) },
      rate_shots   = { w <- recency_weights(season); sum(w * r_shots) },
      rate_hits    = { w <- recency_weights(season); sum(w * r_hits) },
      rate_blocks  = { w <- recency_weights(season); sum(w * r_blocks) },
      rate_pim     = { w <- recency_weights(season); sum(w * r_pim) },
      .groups = "drop"
    ) %>%
    mutate(
      proj_goals   = round(rate_goals * proj_gp, 1),
      proj_assists = round(rate_assists * proj_gp, 1),
      proj_points  = round((rate_goals + rate_assists) * proj_gp, 1),
      proj_shots   = round(rate_shots * proj_gp, 1),
    ...
    select(player_id, player_name, position, n_seasons, proj_gp,
           proj_goals, proj_assists, proj_points, proj_shots, proj_hits, proj_blocks, proj_pim)
}
...
project_goalie_stats <- function(hist_df) {
  ...
  rates %>%
    ...
    summarise(
      ...
      proj_gp     = { w <- recency_weights(season); min(60L, round(sum(w * gp))) },
      proj_gaa    = { w <- recency_weights(season); round(sum(w * coalesce(gaa, median(gaa, na.rm=TRUE))), 2) },
      proj_sv_pct = { w <- recency_weights(season); round(sum(w * coalesce(sv_pct, median(sv_pct, na.rm=TRUE))), 3) },
      rate_wins     = { w <- recency_weights(season); sum(w * r_wins) },
      rate_losses   = { w <- recency_weights(season); sum(w * r_losses) },
      rate_otl      = { w <- recency_weights(season); sum(w * r_otl) },
      rate_shutouts = { w <- recency_weights(season); sum(w * r_shutouts) },
      .groups = "drop"
    ) %>%
    mutate(
