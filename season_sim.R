...
if (!is.null(team_onice_blend)) team_offense <- team_offense %>% left_join(team_onice_blend, by = "team_abbrev")
team_offense <- team_offense %>%
  mutate(shots_against_pg = coalesce(if ("ca_pg_blend" %in% names(.)) ca_pg_blend else NA_real_, shots_against_pg_fallback))
