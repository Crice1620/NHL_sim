...
#   - shots_against_pg is a genuine approximation. Real shot-suppression
#     needs on-ice tracking data (who was on the ice when the other team
#     shot), which this app doesn't have. Instead it's estimated from a
#     blocks/hits-based defensive proxy, scaled around the league-average
#     shot volume. Treat shots_against as the weakest link in this engine.
...
DEF_PROXY_SCALE <- 3         # max approx +/- shots/game swing from the blocks/hits defensive proxy — only used as a fallback now, see real on-ice data below
...
# Moved up from the schedule-template section below so historical on-ice
# data (which may still have old franchise abbreviations, e.g. Arizona's
# last season before relocating) gets remapped BEFORE grouping by team —
# without this, a leftover "ARI" row shows up as an extra, unmatched team
# instead of correctly rolling into Utah's history.
...
# ── Real on-ice team shot-suppression data (replaces the blocks/hits proxy
# wherever available) ────────────────────────────────────────────────────
# The blocks/hits proxy above has no real connection to actual defensive
# quality — a team can rack up hits and blocks while still getting outshot
# badly, or play a clean positional system with modest hit/block totals and
# suppress shots very well. That mismatch was the root cause of team
# strength getting compressed toward the mean regardless of real ability.
# team_onice.csv (from the same play-by-play reconstruction the live app
# uses) gives REAL observed Corsi-against at 5v5. This is a genuine,
# disclosed departure from "100% roster-driven" for this one input — it
# reflects the team's actual on-ice shot suppression over its most recent
# seasons (coaching system/structure carries over somewhat independent of
# exact personnel), recency-weighted so a team that overhauled its system
# still mostly reflects its MOST RECENT season. Falls back to the blocks/
# hits proxy for any team without enough backfilled on-ice history.
team_onice_hist <- lapply(recent3, function(s) {
...
team_onice_hist <- bind_rows(Filter(Negate(is.null), team_onice_hist))
if (nrow(team_onice_hist) > 0)
  team_onice_hist$team_abbrev <- ifelse(team_onice_hist$team_abbrev %in% names(abbrev_fix),
                                        abbrev_fix[team_onice_hist$team_abbrev], team_onice_hist$team_abbrev)

if (nrow(team_onice_hist) > 0) {
  team_onice_blend <- team_onice_hist %>%
    filter(coalesce(gp_onice, 0) > 0) %>%
    mutate(ca_pg = ca_5v5 / gp_onice, cf_pg = cf_5v5 / gp_onice) %>%
    group_by(team_abbrev) %>%
    arrange(season, .by_group = TRUE) %>%
    summarise(
      n_seasons_onice = dplyr::n(),
      ca_pg_blend     = { w <- recency_weights(season); sum(w * ca_pg) },
      cf_pg_blend     = { w <- recency_weights(season); sum(w * cf_pg) },
      .groups = "drop"
    )
  # IMPORTANT: ca_5v5/cf_5v5 are CORSI (shots-on-goal + missed + blocked
  # combined) — a bigger number than shots_for_pg, which comes from real
  # box-score SHOTS ONLY (summed player shot rates). Using raw Corsi
  # directly as "shots_against_pg" would mix two different scales in the
  # same formula (this was a real bug — every team's shots-against looked
  # inflated regardless of actual quality). Fix: derive a SOG-to-Corsi
  # conversion ratio from OUR OWN data (real league-wide shots_for_pg vs
  # real league-wide Corsi-for), then apply that same ratio to Corsi-against
  # so both sides of the shot-suppression formula are on the same scale.
  # Self-calibrating — no guessed constant.
  sog_to_corsi_ratio <- mean(team_offense$shots_for_pg, na.rm = TRUE) / mean(team_onice_blend$cf_pg_blend, na.rm = TRUE)
  cat("  SOG-to-Corsi conversion ratio (self-calibrated):", round(sog_to_corsi_ratio, 3), "\n")
  team_onice_blend <- team_onice_blend %>% mutate(shots_against_onice = ca_pg_blend * sog_to_corsi_ratio)
  cat("  Real on-ice shot-suppression data found for", nrow(team_onice_blend), "of", length(net_lookup), "teams",
      "(recency-weighted across", paste(recent3, collapse=", "), ").\n")
} else {
  team_onice_blend <- NULL
  cat("  No on-ice team data found for any of", paste(recent3, collapse=", "),
      "— falling back to the blocks/hits proxy for shots-against on every team.\n")
}

if (!is.null(team_onice_blend)) team_offense <- team_offense %>% left_join(team_onice_blend %>% select(team_abbrev, shots_against_onice), by = "team_abbrev")
team_offense <- team_offense %>%
  mutate(shots_against_pg = coalesce(if ("shots_against_onice" %in% names(.)) shots_against_onice else NA_real_, shots_against_pg_fallback))
...
for (i in seq_len(nrow(diag_tbl))) {
  r <- diag_tbl[i, ]
  cat(sprintf("  %-4s | shots_for=%.1f shooting_pct=%.4f shots_against=%.1f goalie_sv=%.4f | approx_quality=%.3f\n",
              r$team_abbrev, r$shots_for_pg, r$shooting_pct, r$shots_against_pg, r$goalie_sv_pct, r$approx_quality))
}
cat("\n")
