...
# =============================================================================
# SECTION 6b — Playoff results (derived from bracket API)
# =============================================================================

fetch_playoff_results_scraper <- function(season_end_yr, debug=FALSE) {
  sid_str <- as.character(season_id(season_end_yr))
  url     <- paste0("https://api-web.nhle.com/v1/playoff-series/carousel/", sid_str)
  raw     <- tryCatch({
    resp <- GET(url, timeout(25))
    if (status_code(resp) != 200) return(NULL)
    fromJSON(content(resp, "text", encoding="UTF-8"), simplifyVector=FALSE)
  }, error=function(e) NULL)
  if (is.null(raw) || length(raw$rounds %||% list()) == 0) return(NULL)
  
  all_rows <- list()
  for (rnd_obj in raw$rounds) {
    for (s in rnd_obj$series %||% list()) {
      tryCatch({
        rn  <- as.integer(s$roundNumber %||% NA)
        ta  <- as.character(s$topSeed$abbrev    %||% NA)
        ba  <- as.character(s$bottomSeed$abbrev %||% NA)
        tw  <- as.integer(s$topSeed$wins    %||% 0L)
        bw  <- as.integer(s$bottomSeed$wins %||% 0L)
        wid <- s$winningTeamId %||% NULL
        if (is.na(ta) && is.na(ba)) return(NULL)
        # Determine winner
        winner <- if (!is.null(wid) && length(wid) > 0 && !is.na(wid)) {
          if (tw == 4L) ta else if (bw == 4L) ba else NA_character_
        } else if (tw == 4L) ta else if (bw == 4L) ba else NA_character_
        loser  <- if (!is.na(winner)) if (winner == ta) ba else ta else NA_character_
        result <- switch(as.character(rn),
                         "1" = "1st Round",
                         "2" = "2nd Round",
                         "3" = "Conf Final",
                         "4" = "Stanley Cup Final",
                         NA_character_)
        if (!is.na(winner) && !is.na(loser) && !is.na(result)) {
          # Winner advances, loser gets this round as their result
          all_rows[[length(all_rows)+1]] <- data.frame(
            season=season_end_yr, team_abbrev=loser, result=result,
            stringsAsFactors=FALSE)
          # If this is the final (round 4), winner is Champion
          if (rn == 4L) {
            all_rows[[length(all_rows)+1]] <- data.frame(
              season=season_end_yr, team_abbrev=winner, result="Champion",
              stringsAsFactors=FALSE)
          }
        }
      }, error=function(e) NULL)
...
run_stats_scraper <- function(seasons_override = NULL,
  ...
  for (szn in seasons) {
    ...
    # Playoff results: run manually once after playoffs end each year
    # run_stats_scraper(seasons_override=c(szn), playoffs_only=TRUE)
    # Not included in nightly scrape
...
#' Scrape playoff results only (run once after playoffs end each year)
#' @param seasons integer vector e.g. c(2025). NULL = all seasons.
run_playoff_scraper <- function(seasons_override = NULL, push = TRUE, debug = FALSE) {
  if (push && nchar(trimws(GITHUB_PAT)) == 0)
    stop("GITHUB_PAT is empty.")
  seasons <- if (!is.null(seasons_override)) as.integer(seasons_override)
  else all_stats_seasons()
  for (szn in seasons) {
    log_msg(sprintf("Scraping playoff results for %d-%d...", szn-1L, szn))
    df <- fetch_playoff_results_scraper(szn, debug)
    if (is.null(df) || nrow(df) == 0) { log_msg("  No playoff data yet."); next }
    log_msg(sprintf("  -> %d teams", nrow(df)))
    if (push) github_upload_df(df, paste0(szn_folder(szn), "/playoff_results.csv"))
  }
  log_msg("Done.")
}
...
# =============================================================================
# Goalie Edge scraper additions (append to nhl_edge_scraper.R instead)
# =============================================================================